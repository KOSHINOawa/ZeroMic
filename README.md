...
当你台式机没有麦克风，或者需要高质量的语音输入来进行游戏开黑（如 Discord、KOOK）或网络会议时，只需运行 ZeroMic，用手机扫码或访问局域网地址，即可利用手机的高性能麦克风进行毫秒级延迟的音频传输。

核心采用 P2P **WebRTC 技术**，音频数据完全在局域网内直连，**不经过任何外部服务器**，保证极致的安全与极低的延迟。

## ✨ 核心特性

- 🚀 **开箱即用**：单文件绿色免安装版 `.exe`，双击即刻运行。
- 🔧 **底层自动化**：全自动下载、静默安装并配置 `VB-Audio Virtual Cable` 虚拟声卡，无需用户手动干预。
- 🛡️ **防干扰机制**：独家 COM 接口注入技术，防止 Windows 自动篡改系统默认播放设备，保护你的听觉体验。
- ⚡ **超低延迟**：基于 WebRTC 协议，局域网内提供接近零延迟的高保真音频流。
- 🎨 **现代 UI**：基于 MDUI 打造的沉浸式暗黑风格界面，支持丝滑的动画与状态反馈。
- 🧹 **一键卸载**：内置驱动清理功能，不留任何注册表垃圾。
